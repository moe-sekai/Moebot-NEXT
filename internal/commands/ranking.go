...
func registerRankingBoardCommands(deps *Deps) {
	for _, cmd := range withWorldLinkCommands(parserCommands(deps, "榜线")) {
		...
		zero.OnCommand(cmd.Name).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			payload := renderer.BuildRankingListPayloadWithStore(title, view, runtime.Store, runtime.Assets)
...
func registerRankingChurnCommands(deps *Deps) {
	for _, cmd := range withWorldLinkCommands(parserCommands(deps, "查房")) {
		...
		zero.On("message", rankingCommandRule(cmd.Name, true)).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			view := *board
			view.Rankings = entries
			payload := renderer.BuildChurnRankingListPayloadWithStore(view, runtime.Store, runtime.Assets)
...
func registerWaterTableCommands(deps *Deps) {
	for _, cmd := range withWorldLinkCommands(parserCommands(deps, "查水表")) {
		...
		zero.On("message", rankingCommandRule(cmd.Name, true)).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			payload := renderer.BuildWaterTablePayloadWithStore(*board, entries[0], runtime.Store, runtime.Assets)
...
func formatWaterTableText(payload renderer.WaterTablePayload) string {
	entry := payload.Entry
...
func formatForecastText(payload renderer.ForecastRankingPayload) string {
	lines := []string{payload.Title}
	if payload.Subtitle != "" {
		lines = append(lines, payload.Subtitle)
	}
