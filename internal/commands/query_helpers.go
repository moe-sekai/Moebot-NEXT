...
func cardPayloads(store *masterdata.Store, resolver *assets.Resolver, cards []masterdata.CardInfo) []renderer.CardDetailPayload {
	...
	for _, card := range cards {
		out = append(out, renderer.BuildCardDetailPayloadWithAssets(store, card, resolver))
	}
	return out
}

func musicPayloads(store *masterdata.Store, resolver *assets.Resolver, musics []masterdata.MusicInfo) []renderer.MusicDetailPayload {
	out := make([]renderer.MusicDetailPayload, 0, len(musics))
	for _, music := range musics {
		out = append(out, renderer.BuildMusicDetailPayloadWithAssets(store, music, resolver))
	}
	return out
}

func eventPayloads(store *masterdata.Store, resolver *assets.Resolver, events []masterdata.EventInfo) []renderer.EventInfoPayload {
	out := make([]renderer.EventInfoPayload, 0, len(events))
	for _, event := range events {
		out = append(out, renderer.BuildEventInfoPayloadWithAssets(store, event, resolver))
	}
	return out
}

func gachaPayloads(store *masterdata.Store, resolver *assets.Resolver, gachas []masterdata.GachaInfo) []renderer.GachaInfoPayload {
	out := make([]renderer.GachaInfoPayload, 0, len(gachas))
	for _, gacha := range gachas {
		out = append(out, renderer.BuildGachaInfoPayloadWithAssets(store, gacha, resolver))
	}
	return out
}
