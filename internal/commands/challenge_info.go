...
func RegisterChallengeInfo(deps *Deps) {
	for _, cmd := range parserCommands(deps, "挑战信息") {
		...
		zero.OnCommand(commandName).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			payload.Sections = []renderer.SuiteSectionPayload{{Title: "每日挑战 Live", Kind: "challenge_info", Note: "参考 lunabot：按角色统计挑战等级、最高分，以及未领取高分奖励中的水晶/碎片。", Rows: rows, Extra: sectionExtra}}
