...
func RegisterVirtualLive(deps *Deps) {
	for _, cmd := range parserCommands(deps, "查演唱会") {
		...
		zero.OnCommand(commandName).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			payload := renderer.BuildVirtualLiveListPayloadWithAssets("虚拟 Live", virtualLiveSubtitle(keyword), lives, runtime.Store, runtime.Assets, page, totalPages, total)
...
func formatVirtualLiveListText(payload renderer.VirtualLiveListPayload) string {
