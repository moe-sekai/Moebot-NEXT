...
func musicProgressSummaryFromProfile(profile musicOverviewProfile, store *masterdata.Store) musicProgressSummary {
	...
	return musicProgressSummary{
		Rows:       rows,
		LevelRows:  levelRows,
		Stats:      []renderer.SuiteStatPayload{{Label: "游玩", Value: formatInt(totalPlayed)}, {Label: "Clear", Value: formatInt(totalClear)}, {Label: "FC", Value: formatInt(totalFC)}, {Label: "AP", Value: formatInt(totalAP)}},
...
func rowsFromChallenge(profile challengeProfile, store *masterdata.Store, limit int) ([]renderer.SuiteSectionRowPayload, []renderer.SuiteStatPayload, map[string]interface{}) {
	...
	stats := []renderer.SuiteStatPayload{{Label: "角色数", Value: formatInt(activeCount)}, {Label: "最高分", Value: formatInt(maxScore)}, {Label: "剩余水晶", Value: formatInt(totalRemainJewel)}, {Label: "剩余碎片", Value: formatInt(totalRemainFragment)}, {Label: "剩余奖励档", Value: formatInt(totalRemainRewards)}}
...
func rowsFromEventRecord(profile eventRecordProfile, store *masterdata.Store, resolver interface{ GetEventBannerURL(string) string }, limit int) ([]renderer.SuiteSectionPayload, []renderer.SuiteStatPayload) {
...
func rowsFromLeaderCount(profile leaderCountProfile, store *masterdata.Store, limit int) ([]renderer.SuiteSectionRowPayload, []renderer.SuiteStatPayload, map[string]interface{}) {
	...
	stats := []renderer.SuiteStatPayload{{Label: "总队长次数", Value: formatInt(total)}, {Label: "剩余总次数", Value: formatInt(totalRemain)}, {Label: "普通档位", Value: fmt.Sprintf("%d/%d", totalMissionLevel, totalMissionMax)}, {Label: "剩余档位", Value: formatInt(totalMissionRemain)}, {Label: "EX总次数", Value: formatInt(totalEx)}, {Label: "角色数", Value: formatInt(activeCount)}}
	return out, stats, extra
}
...
func rowsFromMusicReward(profile musicRewardProfile, store *masterdata.Store, limit int) ([]renderer.SuiteSectionRowPayload, []renderer.SuiteStatPayload) {
	...
	return summary.TopRows, []renderer.SuiteStatPayload{{Label: "S评级剩余", Value: formatInt(summary.RankJewelRemain)}, {Label: "连击剩余", Value: formatRewardTotal(summary.ComboRows)}, {Label: "涉及歌曲", Value: formatInt(summary.AchievedMusicCount)}}
}

func sectionsFromMusicReward(profile musicRewardProfile, store *masterdata.Store, limit int) ([]renderer.SuiteSectionPayload, []renderer.SuiteStatPayload) {
	reward := musicRewardSummaryFromProfile(profile, store, limit)
	sections := musicRewardSections(reward)
	return sections, []renderer.SuiteStatPayload{{Label: "S评级剩余", Value: formatInt(reward.RankJewelRemain)}, {Label: "剩余连击奖励", Value: formatRewardTotal(reward.ComboRows)}, {Label: "有效歌曲", Value: formatInt(reward.ValidMusicCount)}}
}

func sectionsFromMusicOverview(profile musicOverviewProfile, store *masterdata.Store, limit int) ([]renderer.SuiteSectionPayload, []renderer.SuiteStatPayload) {
	...
	stats = append(stats,
		renderer.SuiteStatPayload{Label: "S评级剩余", Value: formatInt(reward.RankJewelRemain)},
		renderer.SuiteStatPayload{Label: "连击剩余", Value: formatRewardTotal(reward.ComboRows)},
	)
	return sections, stats
}

func musicRewardSections(summary musicRewardSummary) []renderer.SuiteSectionPayload {
	sections := []renderer.SuiteSectionPayload{{
		Title: "歌曲评级奖励(S)",
	...
	if len(summary.ComboRows) > 0 {
		sections = append(sections, renderer.SuiteSectionPayload{Title: "连击奖励剩余", Kind: "music_reward_combo", Rows: summary.ComboRows, Extra: musicRewardComboExtra(summary)})
	}
...
func musicRewardTopRows(store *masterdata.Store, achievements map[int]map[int]bool, limit int) []renderer.SuiteSectionRowPayload {
	...
	for i := 0; i < limit; i++ {
		row := rows[i]
		out = append(out, renderer.SuiteSectionRowPayload{Rank: i + 1, Label: musicDisplayName(store, row.MusicID), Value: formatInt(row.Count), Meta: "已达成奖励", MusicID: row.MusicID})
	}
	return out
}

func musicRewardComboRows(store *masterdata.Store, musics []masterdata.MusicInfo, achievements map[int]map[int]bool) []renderer.SuiteSectionRowPayload {
...
func musicProgressLevelRows(counts map[string]map[int]*musicProgressCount) []renderer.SuiteSectionRowPayload {
	rows := []renderer.SuiteSectionRowPayload{}
	for _, diff := range musicDifficultyOrder() {
		byLevel := counts[diff]
		if len(byLevel) == 0 {
...
func eventRecordRow(store *masterdata.Store, resolver interface{ GetEventBannerURL(string) string }, eventID int, point int, rank int, characterID int, chapterNo int, order int) renderer.SuiteSectionRowPayload {
...
func rewardTotals(rows []renderer.SuiteSectionRowPayload) (int, int) {
	jewel, shard := 0, 0
...
func formatRewardTotal(rows []renderer.SuiteSectionRowPayload) string {
