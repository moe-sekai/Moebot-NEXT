...
func RegisterMusicReward(deps *Deps) {
	for _, cmd := range parserCommands(deps, "歌曲奖励") {
		...
		zero.OnCommand(commandName).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			if err := runtime.Suite.GetUserData(user.GameID, "", musicRewardFields(), &profile); err != nil {
				ctx.SendChain(message.Text(fmt.Sprintf("获取你的%s Haruki Suite 公开数据失败\n%s", runtime.Label, err.Error())))
				return
			}
			payload := buildSuitePanel(runtime, suitePanelTitle(runtime, "歌曲奖励"), "", profile)
			payload.Subtitle = suitePanelSubtitle(profile.BaseProfile)
			rows, stats := rowsFromMusicReward(profile, runtime.Store, musicRewardDefaultLimit)
			payload.Stats = append(suiteBasicStats(profile.commonSuiteProfile()), stats...)
			payload.Sections = []renderer.SuiteSectionPayload{{Title: "歌曲奖励达成", Rows: rows}}
			sendSuitePanelOrText(ctx, deps, payload, formatMusicRewardText(runtime.Region, profile, musicRewardDefaultLimit))
