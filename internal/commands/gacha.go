	...
	for _, cmd := range parserCommands(deps, command) {
		...
		zero.OnCommand(commandName).SetBlock(true).Handle(func(ctx *zero.Ctx) {
			...
			if listMode || len(gachas) > 1 {
				payload := renderer.BuildGachaListPayloadWithAssets("卡池查询", gachaSubtitleForCommand(keyword), gachas, runtime.Store, runtime.Assets, page, totalPages, total)
...
func formatGachaListText(payload renderer.GachaListPayload) string {
