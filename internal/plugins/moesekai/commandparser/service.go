...
func (s *Service) RenderWithOptions(input string, width int, height int, options RenderOptions) (*renderer.PreviewRenderResult, ParseResult, error) {
	...
	if parsed.Selected != nil && parsed.Selected.Payload != nil && def.Template != "" {
		if def.ID == "chart-detail" {
			if payload, ok := parsed.Selected.Payload.(renderpayloads.MusicDetailPayload); ok && payload.ChartURL != "" {
...
func (s *Service) buildRealtimePayload(def Definition, region string, argument string) (realtimePayloadResult, bool) {
	...
	case "ranking-list":
		...
		payload := renderpayloads.BuildRankingListPayloadWithStore("活动榜线", view, store, resolver)
	...
	case "ranking-target":
		...
		payload := renderpayloads.BuildRankingListPayloadWithStore("sk", view, store, resolver)
		payload.Subtitle = rankingSubtitleForParser(region, view)
	...
	case "churn-ranking":
		...
		view := *board
		view.Rankings = entries
		payload := renderpayloads.BuildChurnRankingListPayloadWithStore(view, store, resolver)
		payload.Title = "查房"
		payload.Subtitle = rankingSubtitleForParser(region, view)
	...
	case "water-table":
		...
		payload := renderpayloads.BuildWaterTablePayloadWithStore(*board, entries[0], store, resolver)
	...
	case "forecast-ranking":
		...
		payload := renderpayloads.BuildForecastRankingPayload(*board, event.Name, region, config.RegionLabel(region))
...
func templateForPayload(fallback string, payload any) string {
	switch payload.(type) {
	case renderpayloads.CardListPayload:
		return "card_list"
	case renderpayloads.MusicListPayload:
		return "music_list"
	case renderpayloads.Best30Payload:
		return "best30"
	case renderpayloads.EventListPayload:
		return "event_list"
	case renderpayloads.GachaListPayload:
		return "gacha_list"
	case renderpayloads.VirtualLiveListPayload:
		return "virtual_live_list"
	case renderpayloads.WaterTablePayload:
		return "water_table"
	case renderpayloads.ForecastRankingPayload:
...
func selectedDifficultyPayloadForParser(payload renderpayloads.MusicDetailPayload, diff string, chartSourceURL string) renderpayloads.MusicDetailPayload {
	if diff == "" {
		diff = "master"
	}
...
func searchAndBuild(def Definition, store *masterdata.Store, resolver *assets.Resolver, aliases map[int]assets.MusicAlias, chartSourceURL string, argument string) ([]EntityResult, *EntityResult) {
	...
	case SearchTypeCard:
		...
		if result.Mode == cardquery.ModeList {
			payload := renderpayloads.BuildCardListPayloadWithAssets("卡牌查询", commandListSubtitle(argument), cards, store, resolver, result.Page, result.TotalPages, result.Total)
		...
		payload := renderpayloads.BuildCardDetailPayloadWithAssets(store, cards[0], resolver)
		selected := rows[0]
	...
	case SearchTypeMusic:
		...
		if result.Mode == musicsearch.ModeList {
			payload := renderpayloads.BuildMusicListPayloadWithAssets("曲目查询", commandListSubtitle(query), result.Musics, store, resolver, result.Page, result.TotalPages, result.Total)
		...
		payload := renderpayloads.BuildMusicDetailPayloadWithAssets(store, *result.Music, resolver)
	...
	case SearchTypeEvent:
		...
		if len(events) > 1 {
			...
			payload := renderpayloads.BuildEventListPayloadWithAssets("活动查询", commandListSubtitle(argument), shown, store, resolver, 1, (len(events)+11)/12, len(events))
		...
		payload := renderpayloads.BuildEventInfoPayloadWithAssets(store, events[0], resolver)
		selected := rows[0]
		selected.Payload = payload
		return rows, &selected
	case SearchTypeGacha:
		...
		payload := renderpayloads.BuildGachaInfoPayloadWithAssets(store, gachas[0], resolver)
		selected := rows[0]
		selected.Payload = payload
		return rows, &selected
	case SearchTypeVirtualLive:
		...
		payload := renderpayloads.BuildVirtualLiveListPayloadWithAssets("虚拟 Live", "近期演唱会", lives, store, resolver, 1, 1, len(lives))
		selected := rows[0]
		selected.Payload = payload
		return rows, &selected
	default:
