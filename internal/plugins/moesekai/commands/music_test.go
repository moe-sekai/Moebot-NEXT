...
func TestSelectedDifficultyPayloadUsesChartSourceTemplate(t *testing.T) {
	payload := selectedDifficultyPayload(renderpayloads.MusicDetailPayload{
		ID:           739,
		Difficulties: []renderpayloads.MusicDifficultyPayload{{MusicDifficulty: "master", Difficulty: "master", PlayLevel: 32}},
...
func TestBuildChartRenderRequestUsesChartTemplate(t *testing.T) {
	req := buildChartRenderRequest(renderpayloads.MusicDetailPayload{ID: 74, Title: "独りんぼエンヴィー"})
...
func TestFormatChartTextShowsChartFocusedDifficultyRows(t *testing.T) {
	text := formatChartText(renderpayloads.MusicDetailPayload{
		ID:         88,
		Title:      "テスト楽曲",
		Categories: []string{"mv", "original"},
		Difficulties: []renderpayloads.MusicDifficultyPayload{
