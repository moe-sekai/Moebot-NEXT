...
func TestFormatCardTextIncludesRelatedEvents(t *testing.T) {
	text := formatCardText(renderpayloads.CardDetailPayload{
		ID:             1001,
		Prefix:         "测试卡牌",
		CharacterName:  "初音未来",
		CardRarityType: "rarity_4",
		Attr:           "cute",
		Events: []renderpayloads.CardEventPayload{{
...
func TestFormatEventTextIncludesBonusCards(t *testing.T) {
	text := formatEventText(renderpayloads.EventInfoPayload{
		ID:        2001,
		Name:      "测试活动",
		EventType: "marathon",
		BonusCards: []renderpayloads.CardDetailPayload{{
