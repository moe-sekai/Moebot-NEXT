...
export async function getCurrentAdmin(): Promise<{
  ...
  const { data } = await api.get<{ username: string; nickname: string }>(
    "/auth/me",
  );
  return data;
}
...
export async function preloadRendererCardThumbnails(
  region?: string,
  force = false,
) {
...
export async function getRecentCommands(limit = 10, clientId?: string) {
  const { data } = await api.get<RecentCommandsResponse>("/commands/recent", {
    params: { limit, ...clientParam(clientId) },
  });
  return data;
}
...
export async function renderParsedCommand(
  ...
  return {
    ...
    timings: {
      ...
      image_remote: parseHeaderNumber(
        response.headers["x-render-image-remote"],
      ),
      image_cache_hits: parseHeaderNumber(
        response.headers["x-render-image-cache-hits"],
      ),
      image_cache_misses: parseHeaderNumber(
        response.headers["x-render-image-cache-misses"],
      ),
      image_cache_errors: parseHeaderNumber(
        response.headers["x-render-image-cache-errors"],
      ),
    },
  };
}
...
export async function getGroups(
  ...
  const { data } = await api.get<GroupsResponse>("/groups", {
    params: { page, limit, stats_days: statsDays, ...clientParam(clientId) },
  });
  return data;
}
...
export async function getCommandStats(days = 7, top = 10, clientId?: string) {
  const { data } = await api.get<CommandStatsResponse>("/stats/commands", {
    params: { days, top, ...clientParam(clientId) },
  });
  return data;
}
...
export async function listFilterTemplates() {
  const { data } =
    await api.get<FilterTemplateListResponse>("/filter/templates");
  return data.items;
}

export async function getFilterTemplate(id: number) {
  const { data } = await api.get<FilterTemplatePayload>(
    `/filter/templates/${id}`,
  );
  return data;
}
...
function clientParam(clientId?: string) {
...
export async function listMarketPlugins(
  refresh = false,
): Promise<MarketPluginListResponse> {
...
export async function getAutochatOverview() {
  const { data } = await api.get<AutochatOverview>(
    "/plugins/autochat/overview",
  );
  return data;
}
...
export async function getAutochatProviders() {
  const { data } = await api.get<AutochatProviders>(
    "/plugins/autochat/providers",
  );
  return data;
}

export async function updateAutochatProviders(payload: AutochatProviders) {
  const { data } = await api.put<AutochatProviders>(
    "/plugins/autochat/providers",
    payload,
  );
  return data;
...
export async function updateAutochatPersona(payload: AutochatPersona) {
  const { data } = await api.put<AutochatPersona>(
    "/plugins/autochat/persona",
    payload,
  );
  return data;
}
...
export async function getAutochatTriggers() {
  const { data } = await api.get<AutochatTriggers>(
    "/plugins/autochat/triggers",
  );
  return data;
}

export async function updateAutochatTriggers(payload: AutochatTriggers) {
  const { data } = await api.put<AutochatTriggers>(
    "/plugins/autochat/triggers",
    payload,
  );
  return data;
...
export async function testAutochatProvider(payload: {
  ...
  const { data } = await api.post<TestProviderResult>(
    "/plugins/autochat/test-provider",
    payload,
  );
  return data;
}
...
export async function getAutochatYAML() {
  const { data } = await api.get<{ path: string; yaml: string }>(
...
export async function upsertAutochatTemplate(
  name: string,
  payload: AutochatTemplate,
) {
...
export async function updateAutochatYAML(yaml: string) {
  const { data } = await api.put<{ ok: boolean }>("/plugins/autochat/yaml", {
    yaml,
  });
  return data;
}
...
export async function listGalleries() {
  const { data } = await api.get<{ galleries: GalleryDTO[] }>(
    "/plugins/gallery/galleries",
  );
...
export async function deleteGallery(name: string) {
  const { data } = await api.delete<{ ok: boolean }>(
    `/plugins/gallery/galleries/${encodeURIComponent(name)}`,
  );
  return data;
}
...
export async function listGalleryUploadRecords(
  params: {
    user_id?: number;
    group_id?: number;
    gallery?: string;
  ...
  const { data } = await api.get<{
    records: GalleryUploadRecord[];
    total: number;
  }>("/plugins/gallery/upload-records", { params });
  return data;
}
...
export async function listGalleryPics(name: string, offset = 0, limit = 100) {
  const { data } = await api.get<{ pics: GalleryPic[]; total: number }>(
    `/plugins/gallery/galleries/${encodeURIComponent(name)}/pics`,
    {
      params: { offset, limit },
    },
  );
...
export async function deleteGalleryPic(pid: number) {
  const { data } = await api.delete<{ ok: boolean }>(
    `/plugins/gallery/pics/${pid}`,
  );
  return data;
}

export async function uploadGalleryPic(
  galleryName: string,
  file: File,
  checkDup = true,
) {
  ...
  const { data } = await api.post<{ ok: boolean; pid: number }>(
    "/plugins/gallery/upload",
    form,
    {
