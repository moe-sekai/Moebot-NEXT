<template>
  <main class="page-stack">
    <PageHeader eyebrow="Groups" title="群组管理" subtitle="查看已交互群组、命令使用统计，管理启停状态、备注与每群配置 (JSON)。">
      <template #actions>
        <UiButton variant="outline" size="sm" :loading="loading" @click="loadGroups">刷新</UiButton>
      </template>
    </PageHeader>
    ...
    <UiCard>
      <div class="card-heading">
        <div>
          <h2>群组列表</h2>
          <p>共 {{ total }} 个群；最近 {{ statsDays }} 天命令统计随列表附带。</p>
        </div>
        <div class="groups-toolbar">
          <label class="groups-toolbar__field">
            <span>关键词</span>
            <input v-model.trim="filterKeyword" class="ui-input" placeholder="按群号 / 名称过滤" />
          </label>
          <label class="groups-toolbar__field">
            <span>平台</span>
          ...
          <label class="groups-toolbar__field">
            <span>统计天数</span>
            <select v-model.number="statsDays" class="ui-select" @change="loadGroups">
      ...
      <div v-else-if="filteredGroups.length === 0" class="empty-state">
        ...
        <div>
          ...
          <p>当机器人在群中收到首条命令时会自动登记。</p>
        </div>
      </div>
      <div v-else class="groups-list">
        <article v-for="group in filteredGroups" :key="group.id" class="group-row" :class="{ 'group-row--disabled': !group.enabled }">
          <header class="group-row__head">
            <div class="group-row__title">
              <UiBadge :variant="group.enabled ? 'success' : 'outline'">{{ group.enabled ? '启用' : '停用' }}</UiBadge>
              <strong>{{ group.name || '未命名群' }}</strong>
              <span class="group-row__sub">{{ group.platform }} · {{ group.group_id }}</span>
            </div>
            <div class="group-row__stats">
              <div><span>命令数（{{ group.stats?.days ?? statsDays }}d）</span><strong>{{ group.stats?.count ?? 0 }}</strong></div>
              <div><span>平均耗时</span><strong>{{ formatAvgMs(group.stats?.avg_ms) }}</strong></div>
              <div><span>最近活跃</span><strong>{{ formatTime(group.stats?.last_used) }}</strong></div>
              <div><span>登记时间</span><strong>{{ formatTime(group.created_at) }}</strong></div>
            </div>
            <div class="group-row__actions">
              <UiButton size="sm" variant="outline" @click="toggleEnabled(group)" :loading="busy[group.id] === 'toggle'">
                {{ group.enabled ? '停用' : '启用' }}
              </UiButton>
              <UiButton size="sm" variant="ghost" @click="toggleExpand(group.id)">
                {{ expandedId === group.id ? '收起' : '编辑' }}
              </UiButton>
              <UiButton size="sm" variant="ghost" @click="loadRecent(group)" :loading="busy[group.id] === 'recent'">
                最近命令
              </UiButton>
              <UiButton size="sm" variant="outline" @click="confirmDelete(group)" :loading="busy[group.id] === 'delete'">
                删除
              </UiButton>
            </div>
          </header>
          ...
          <section v-if="expandedId === group.id" class="group-row__editor">
            <label class="settings-field">
              <span>群备注 / 名称</span>
              <input v-model.trim="draft.name" class="ui-input" placeholder="可选；用于控制台展示" />
            </label>
            <label class="settings-field settings-field--full">
              <span>群配置（JSON）<small style="opacity:0.6">— 留空 / {} 表示默认</small></span>
              <textarea v-model="draft.config" class="ui-input ui-textarea" rows="6" spellcheck="false" placeholder='{"feature_disabled": ["b30"]}'></textarea>
            </label>
            <UiAlert v-if="draftJSONError" variant="warning" title="JSON 无效">{{ draftJSONError }}</UiAlert>
            <div class="settings-actions-row">
              <UiButton size="sm" :disabled="!!draftJSONError" :loading="busy[group.id] === 'save'" @click="saveDraft(group)">保存修改</UiButton>
              <UiButton size="sm" variant="ghost" @click="cancelExpand">取消</UiButton>
            </div>
          </section>

          <section v-if="recentMap[group.id]" class="group-row__recent">
            <header><strong>最近 {{ recentMap[group.id]!.length }} 条命令</strong></header>
            <div v-if="recentMap[group.id]!.length === 0" class="muted-text">暂无命令记录。</div>
            <ul v-else class="group-recent-list">
              <li v-for="row in recentMap[group.id]" :key="row.id">
                <code>{{ row.command }}</code>
                <span class="group-recent-list__meta">用户 {{ row.user_id }} · {{ row.region || '-' }} · {{ row.response_ms }}ms · {{ formatTime(row.created_at) }}</span>
                <span v-if="row.args" class="group-recent-list__args">{{ row.args }}</span>
              </li>
            </ul>
          </section>
        </article>
      ...
      <footer v-if="totalPages > 1" class="groups-pager">
        <UiButton size="sm" variant="outline" :disabled="page <= 1" @click="goPage(page - 1)">上一页</UiButton>
        <span>第 {{ page }} / {{ totalPages }} 页</span>
        <UiButton size="sm" variant="outline" :disabled="page >= totalPages" @click="goPage(page + 1)">下一页</UiButton>
      </footer>
    </UiCard>
  </main>
</template>
...
import type { GroupRecentCommand, GroupRow } from "../api/types";
...
const busy = reactive<Record<number, "toggle" | "save" | "delete" | "recent" | undefined>>({});
...
async function loadGroups() {
	...
	error.value = "";
	try {
		const data = await getGroups(page.value, limit.value, statsDays.value);
...
async function confirmDelete(group: GroupRow) {
	if (!window.confirm(`确认删除群组 ${group.name || group.group_id} 的登记记录？\n该群配置与启停状态会被移除（不会影响命令统计历史）。`))
		return;
